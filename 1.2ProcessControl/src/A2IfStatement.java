...
/**
 * @author Lingmao
 * @creat 2021-10-04  22:58
 */

//来自 https://www.liaoxuefeng.com/wiki/1252599548343744/1259539352677728
/*
...
//    }
//}
//当条件 n >= 60 计算结果为 true 时，if 语句块被执行，
// 将打印 "及格了"，否则，if 语句块将被跳过。修改 n 的值可以看到执行效果。
...
//省略花括号并不总是一个好主意。假设某个时候，突然想给 if 语句块增加一条语句时：
//public class A2IfStatement {
//    public static void main(String[] args) {
//        int n = 50;
//        if (n >= 60)
...
//    }
//}
//执行发现，n = 100 时，满足条件 n >= 90，
// 但输出的不是 "优秀"，而是 "及格了"，
// 原因是 if 语句从上到下执行时，先判断 n >= 60 成功后，
// 后续 else 不再执行，因此，if (n >= 90) 没有机会执行了。
...
//判断引用类型相等
//在 Java 中，判断值类型的变量是否相等，可以使用 == 运算符。
// 但是，判断引用类型的变量是否相等，== 表示 “引用是否相等”，
// 或者说，是否指向同一个对象。例如，下面的两个 String 类型，
// 它们的内容是相同的，但是，分别指向不同的对象，用 == 判断，结果为 false：
